...
export default function HomeScreen() {
  ...
  const handleFaceCaptured = async (imageUri: string, faceBase64: string) => {
    ...
    try {
      // Get tokens from stored authentication
      const tokens = await apiService.getStoredAuthTokens();
      if (!tokens) {
        throw new Error("Authentication tokens not found. Please login again.");
      }

      // For now, just upload without encryption since encryptionKey is not in the response
      // You can add encryption later when the backend provides the key
      await apiService.uploadFaceImage(faceBase64, tokens.accessToken);

      Alert.alert(
        "Success",
        "Face uploaded successfully! Your attendance has been recorded.",
        [{ text: "OK" }]
      );
    } catch (error: any) {
      ...
      Alert.alert(
        "Upload Failed",
        error.message || "Failed to upload face image. Please try again."
      );
    } finally {
      setUploading(false);
    }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.infoSection}>
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Your face data is encrypted before transmission
            </Text>
          </View>
          ...
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Quick face detection and verification process
            </Text>
          </View>
        </View>
        ...
        <View style={styles.instructionsCard}>
          <Text style={styles.instructionsTitle}>
            <Ionicons name="information-circle" size={20} color="#007AFF" /> How
            to use
          </Text>
          <View style={styles.instructionItem}>
            <Text style={styles.instructionNumber}>1.</Text>
            <Text style={styles.instructionText}>
              Press the Scan Face button above
            </Text>
          </View>
          <View style={styles.instructionItem}>
          ...
          <View style={styles.instructionItem}>
            ...
            <Text style={styles.instructionText}>
              Press the capture button to take the photo
            </Text>
          </View>
        </View>
      </ScrollView>
